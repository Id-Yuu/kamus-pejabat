...
const modeEl =
  document.getElementById('translation-mode');

const inputLabelEl =
  document.getElementById('input-label');

const outputLabelEl =
  document.getElementById('output-label');
    ...
    (token, tokenIndex) => {
      ...
      if (token.type === 'switchable') {
        ...
        if (token.alternatives.length > 1) {
          ...
          span.title =
            `Double-click untuk mengganti: ${
              token.alternatives.join(' / ')
            }`;
        }

        span.textContent =
          token.alternatives[token.index];
